...
type SavingsAccount struct {
	Account              // Embed Account strcut anonymus field
...
func (sa *SavingsAccount) AddInterest() {
	...
	err := sa.Deposit(interest) // Users promoted Deposit method
	if err != nil {
		fmt.Printf("AddInterest Error depositing $%2.f to savings account. %v\n", interest, err)
	}
}
...
func (oa *OverdraftAccount) Withdraw(amount float64) error {
	...
	if (oa.Balance + oa.OverdraftLimit) < amount {
		return fmt.Errorf("withdrawal of $%.2f exceeds overdraft limit for %s. Available including overdraft: $%2.f",
...
func main() {
	...
	overAccount := OverdraftAccount{
		Account: Account{
			AccountNumber: "SER004",
			Balance:       100.00,
	...
	fmt.Println(overAccount.String())

	err = overAccount.Deposit(50.00)
	if err != nil {
		fmt.Println("err", err)
	}
